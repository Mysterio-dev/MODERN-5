    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                const iconMenuClicked = e.target.closest(".icon-menu");
                const menuLinkClicked = e.target.closest(".menu__link");
                if (bodyLockStatus && (iconMenuClicked || menuLinkClicked)) {
                    bodyLockToggle();
                    document.documentElement.classList.toggle("menu-open");
                    if (menuLinkClicked) setTimeout((() => {
                        functions_menuClose();
                    }), 300);
                }
            }));
...
***************************************************************************** */
        ...
        if (galleries.length) {
            ...
            galleries.forEach((gallery => {
                const lightGalleryInstance = lightgallery_es5(gallery, {
                    selector: ".gallery__wrapper .gallery__slide img",
                    plugins: [ lg_zoom_min ],
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ymaps.ready((function() {
                var cities = {
                    moscow: {
                        map: createMap("map-moscow", [ 55.755826, 37.6173 ]),
                        items: document.querySelectorAll('.contacts__map-item[data-city="moscow"]')
                    },
                    samara: {
                        map: createMap("map-samara", [ 53.241504, 50.221245 ]),
                        items: document.querySelectorAll('.contacts__map-item[data-city="samara"]')
                    },
                    khujand: {
                        map: createMap("map-khujand", [ 40.292174, 69.623363 ]),
                        items: document.querySelectorAll('.contacts__map-item[data-city="khujand"]')
                    }
                };
                Object.keys(cities).forEach((function(city) {
                    var cityData = cities[city];
                    addClickHandlers(cityData.items, cityData.map);
                }));
                ...
                function addClickHandlers(items, map) {
                    items.forEach((function(item) {
                        item.addEventListener("click", (function() {
                            setActiveState(items, this);
                            var address = this.getAttribute("data-address");
                            geocodeAndSetMap(map, address);
                        }));
                ...
                function setActiveState(items, activeItem) {
                    items.forEach((function(el) {
                        el.classList.remove("_active");
                    }));
                    activeItem.classList.add("_active");
                }
                function geocodeAndSetMap(map, fullAddress) {
                    ymaps.geocode(fullAddress).then((function(res) {
                        ...
                        map.geoObjects.add(new ymaps.Placemark(coordinates, {}, {
                            ...
                            iconImageHref: "img/icons/geo-alt-fill.svg",
                            iconImageSize: [ 50, 50 ]
                        }));
                    }), (function(err) {
                        console.error(err);
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            navbarLinks.forEach((link => {
                link.addEventListener("click", (function(e) {
                    ...
                    const targetId = this.getAttribute("href").substring(1);
                    const targetElement = document.getElementById(targetId);
                    if (targetElement) if ("scrollBehavior" in document.documentElement.style) window.scrollTo({
                        top: targetElement.offsetTop - offset,
                        behavior: "smooth"
                    }); else window.scrollTo(0, targetElement.offsetTop - offset); else console.error(`Element with id "${targetId}" not found`);
                }));
            }));
        }));
        document.addEventListener("DOMContentLoaded", (function() {
            var btnTrigger = document.querySelector(".btn-trigger");
            var mapItems = document.querySelectorAll(".contacts__map-item");
            var mapBlocks = document.querySelectorAll(".contacts__map-block");
            mapBlocks.forEach((function(mapBlock) {
                var mapList = mapBlock.querySelector(".contacts__map-list");
                var btnTrigger = mapBlock.querySelector(".btn-trigger");
                if (mapList && mapList.children.length === 1) btnTrigger.parentNode.removeChild(btnTrigger);
            }));
            function toggleMapItems() {
                btnTrigger.classList.toggle("_active");
                if (btnTrigger.classList.contains("_active")) mapItems.forEach((function(item) {
                    if (item.classList.contains("_active")) item.parentNode.appendChild(item);
                }));
                mapItems.forEach((function(item) {
                    item.style.display = btnTrigger.classList.contains("_active") ? "block" : item.classList.contains("_active") ? "block" : "none";
                }));
            }
            function handleResize() {
                if (window.innerWidth < 767) {
                    mapItems.forEach((function(item) {
                        if (!item.classList.contains("_active")) item.style.display = "none";
                    }));
                    btnTrigger.addEventListener("click", toggleMapItems);
                } else {
                    btnTrigger.removeEventListener("click", toggleMapItems);
                    mapItems.forEach((function(item) {
